...
<div class="flex flex-col lg:flex-row gap-4 min-h-screen bg-gray-50 p-3">
  ...
  <div class="flex-1 min-w-0">
    ...
    <div v-else class="bg-white rounded-xl border overflow-hidden">
      <div class="text-white p-4" style="background:#1e3a5f;">
...
function selectToken(token) {
  ...
  if (token.status === 'waiting') callNext(token);
}
...
function statusBadge(s) {
  return {
    waiting: 'bg-yellow-100 text-yellow-700', 'in-progress': 'bg-blue-100 text-blue-700',
